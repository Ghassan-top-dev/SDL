...
bool init() {
    ...
    if (SDL_Init(SDL_INIT_VIDEO) < 0) {
        ...
        // Create SDL window
        gWindow = SDL_CreateWindow("MEOWCULATOR", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
        if (gWindow == NULL) {
...
bool loadMedia() {
    ...
    soundEffect = Mix_LoadWAV("mewo.wav");
    if (soundEffect == NULL) {
